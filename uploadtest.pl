...
#Saving to a folder outside of the public-html folder
my $save_to_directory = qq~/home/normywebguy/upload~;
my $ScriptURL = qq~http://$ENV{'SERVER_NAME'}$ENV{'SCRIPT_NAME'}~;
...
if($qs{upload} == 1) {
    ...
    my $res1 = &upload($save_to_directory, 'myfield', '', 100, 1);
    my $res2 = &upload($save_to_directory, 'anotherfile', '', 100, 1);
    my $res3 = &upload($save_to_directory, 'FILE3', '', 100, 1);
...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="https://www.yourdomain.com/css/flexbox.css">
  <link rel="stylesheet" type="text/css" href="https://www.yourdomain.com/css/programming_blog.css">
</head>
<body>
<div class="main-page">
  <header>
...
if(opendir(READ, "$save_to_directory")) {
    ...
    foreach my $sorted_file (@sorted) {
        unless(($sorted_file eq '.') || ($sorted_file eq '..')) {
            print qq~<a href="https://www.yourdomain.com/cgi-bin/uploads/download.cgi?d=uploaddemo&f=$sorted_file&pid=$$">$sorted_file</a> &nbsp;&nbsp;\[<a href="$ScriptURL?f=$sorted_file&delete=1&pid=$$" style="color:#800000;">Delete</a>\]<br />\n~;
        }
    }
    print qq~<br /><br />\n~;
}
